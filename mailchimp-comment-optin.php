	...
	class TGM_MailChimp_Comment_Optin {
		...
		public function get_api_creds() {
			...
			<div class="tgm-mc-header">
				<div class="content">
					<?php printf( __( '<p>%s</p>', 'tgm-mc-optin' ), 'You must enter a valid MailChimp API key before you can proceed.' ); ?>
				</div>
			</div>
				...
				<div class="content">
					<table class="form-table">
						<tbody>
							...
							<tr valign="middle">
								<th scope="row">
									<label for="<?php echo $this->option; ?>[get_name]"><?php _e( 'Grab the commenter\'s name too?', 'tgm-mc-optin' ); ?></label>
								</th>
								<td>
									<input id="tgm-mc-get-name" type="checkbox" name="<?php echo $this->option; ?>[get_name]" value="<?php echo $tgm_mc_options['get_name']; ?>" <?php checked( $tgm_mc_options['get_name'], 1 ); ?> />
									<span class="description"><?php _e( 'By default, only the email is sent to the list', 'tgm-mc-optin' ); ?></span>
								</td>
							</tr>
							<tr valign="middle">
								<th scope="row">
								...
								<td>
									<input id="tgm-mc-check-text" type="text" name="<?php echo $this->option; ?>[check_text]" size="60" value="<?php echo $tgm_mc_options['check_text']; ?>" />
								</td>
							</tr>
							<tr valign="middle">
								<th scope="row">
								...
								<td>
									<input id="tgm-mc-subscribed-text" type="text" name="<?php echo $this->option; ?>[subscribed_text]" size="60" value="<?php echo $tgm_mc_options['subscribed_text']; ?>" />
								</td>
							</tr>
							<tr valign="middle">
								<th scope="row">
								...
								<td>
									<input id="tgm-mc-pending-text" type="text" name="<?php echo $this->option; ?>[pending_text]" size="60" value="<?php echo $tgm_mc_options['pending_text']; ?>" />
								</td>
							</tr>
							<tr valign="middle">
								<th scope="row">
								...
								<td>
									<input id="tgm-mc-admin-text" type="text" name="<?php echo $this->option; ?>[admin_text]" size="60" value="<?php echo $tgm_mc_options['admin_text']; ?>" />
								</td>
							</tr>
							<tr valign="middle">
								<th scope="row">
		...
		public function sanitize_options( $input ) {
			...
			$tgm_mc_options['get_name'] = isset( $input['get_name'] ) ? (int) 1 : (int) 0;
		...
		public function comment_form() {
			...
			if ( 'admin' == $this->get_viewer_status() ) {
				echo '<p class="tgm-mc-subscribe" ' . $clear . '>' . $tgm_mc_options['admin_text'] . '</p>';
			}
			elseif ( 'subscribed' == $this->get_viewer_status() ) {
				echo '<p class="tgm-mc-subscribe" ' . $clear . '>' . $tgm_mc_options['subscribed_text'] . '</p>';
			}
			elseif ( 'pending' == $this->get_viewer_status() ) {
				echo '<p class="tgm-mc-subscribe" ' . $clear . '>' . $tgm_mc_options['pending_text'] . '</p>';
			}
			else {
				echo '<p class="tgm-mc-subscribe" ' . $clear . '>';
					echo '<input type="checkbox" name="tgm_mc_get_subscribed" id="tgm-mc-get-subscribed" value="' . esc_attr( $tgm_mc_options['check_text'] ) . '" style="width: auto;" ' . $checked . ' />';
		...
		public function get_viewer_name() {
			...
			$commenter_name = isset( $_COOKIE[sanitize_key( 'comment_author_' . COOKIEHASH )] ) ? trim( $_COOKIE[sanitize_key( 'comment_author_' . COOKIEHASH )] ) : '';
		 ...
		 *
		 * @global array $tgm_mc_options Array of plugin options
		 * @return null|boolean Early if on admin, true on success, false on failure
		 */
		public function populate_list() {
		
			global $tgm_mc_options;
			
			/** Return early if we are in the admin */
			if ( is_admin() )
				return;
			
			/** Only go forward if the checkbox has been selected */
			if ( ! empty( $_COOKIE['tgm_mc_checkbox_' . COOKIEHASH] ) && 'checked' == $_COOKIE['tgm_mc_checkbox_' . COOKIEHASH] && ! $this->get_viewer_status() ) {
				$name = $this->get_viewer_name();
				$email = $this->get_viewer_email();
				$use_name = $tgm_mc_options['get_name'] ? true : false;
				...
				if ( $use_name ) {
					...
					$api->listSubscribe( $tgm_mc_options['current_list_id'], $email, array() );
				}
				return true;
			}
		 ...
		 * @since 1.0.0
		 * 
		 * @param string $cookie The cookie to save
		 * @return string $cookie Amended cookie value (checked or unchecked)
		 */
		public function save_checkbox_state( $cookie ) {
		
			/** If our checked has been checked, set a cookie with the value of 'checked', else 'unchecked' */
